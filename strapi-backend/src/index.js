...
const Security = {
    ...
    sanitizeText(text, maxLength = CONFIG.MAX_MESSAGE_LENGTH) {
        ...
        return text.trim().substring(0, maxLength).replace(/<(?!b|i|code|u|\/b|\/i|\/code|\/u)/g, '&lt;').replace(/(?<!b|i|code|u|\/b|\/i|\/code|\/u)>/g, '&gt;');
    },
...
async function sendMessage(chatId, text, options = {}) {
    try {
        ...
        const response = await axios.post(`${TELEGRAM_API}/sendMessage`, {
            ...
            text: text, // Используем оригинальный текст, санитизация внутри
            parse_mode: 'HTML',
...
/**
 * Отправка уведомления о пользователе в Telegram
 */
async function notifyUserChange(strapi, user, action) {
    try {
        const adminId = CONFIG.ADMIN_ID;
        const status = action === 'create' ? '🆕 <b>НОВЫЙ ПОЛЬЗОВАТЕЛЬ</b>' : action === 'delete' ? '🗑 <b>ПОЛЬЗОВАТЕЛЬ УДАЛЁН</b>' : '🔄 <b>ПОЛЬЗОВАТЕЛЬ ОБНОВЛЁН</b>';

        const message = `${status}\n\n` +
            `🆔 ID: <code>${user.id}</code>\n` +
            `📛 Имя: <b>${user.username || 'Без имени'}</b>\n` +
            `📧 Email: ${user.email || 'Нет'}\n` +
            `📊 Статус: ${user.blocked ? '🚫 Заблокирован' : '✅ Активен'}`;

        await sendMessage(adminId, message);
    } catch (err) {
        console.error('[Notify] Ошибка уведомления:', err.message);
    }
}
...
async function loadUsersFromStrapi(strapi) {
    try {
        ...
        // Если users вернул объект с data, извлекаем массив
        const usersArray = Array.isArray(users) ? users : (users?.data || []);

        SessionStore.allUsers.clear();

        usersArray.forEach(user => {
            // Если user это объект с id напрямую
            const userId = user.id || user.ID;
            if (userId) {
                const safeId = Security.sanitizeId(String(userId));
                if (safeId) {
...
const CommandHandlers = {
    ...
    async users(chatId, userId, args, strapi) {
        ...
        if (count === 0 && SessionStore.allUsers.size === 0) {
            return sendMessage(chatId, '📴 Пользователей пока нет или ошибка базы');
        }

        const usersList = Array.from(SessionStore.allUsers.values()).slice(0, 10);
    ...
    async bootstrap({ strapi }) {
        ...
        if (admin) {
            await sendMessage(admin.id,
                `👑 <b>АДМИН ПАНЕЛЬ ЗАПУЩЕНА</b> v3.2\n\n` +
                `🛡 Безопасность: АКТИВНА\n` +
                `👥 Пользователей: ${SessionStore.allUsers.size}\n` +
                `📦 В очереди: ${SessionStore.pendingProducts.size}\n\n` +
                `/users - Список\n` +
                `/statsall - Статистика\n` +
                `/pending - Товары на модерацию`
            );
        }

        // РЕГИСТРАЦИЯ LIFECYCLES ДЛЯ ПОЛЬЗОВАТЕЛЕЙ
        strapi.db.lifecycles.subscribe({
            models: ['plugin::users-permissions.user'],
            async afterCreate(event) {
                const { result } = event;
                SessionStore.allUsers.set(String(result.id), {
                    id: String(result.id),
                ...
                await notifyUserChange(strapi, result, 'create');
            },
            async afterDelete(event) {
                const { result } = event;
                if (result) {
                    SessionStore.allUsers.delete(String(result.id));
                    await notifyUserChange(strapi, result, 'delete');
                }
            },

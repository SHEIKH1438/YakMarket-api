...
function checkAccess(userId, permission = null) {
    if (!ModeratorsDB.isModerator(userId)) {
        return { allowed: false, reason: '⛔ Вы не являетесь модератором!' };
    }

    if (permission && !ModeratorsDB.hasPermission(userId, permission)) {
        return { allowed: false, reason: '⛔ У вас нет прав для этой команды!' };
    }
...
const CommandHandlers = {
    ...
    async pending(chatId, userId) {
        const access = checkAccess(userId, 'moderate');
        if (!access.allowed) {
    ...
    async user(chatId, userId, args) {
        const access = checkAccess(userId, 'user_info');
        if (!access.allowed) {
            return sendMessage(chatId, access.reason);
        }
